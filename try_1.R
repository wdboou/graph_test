...
# plot 2
jpeg(filename="dolphins_sub1.jpg",width=800, height=800,units='px')
gn <- graph.neighborhood(dol, order=1)
plot(gn[[1]],
     layout=layout)
dev.off()

# plot 3
jpeg(filename="dolphins_sub2.jpg",width=800, height=800,units='px')
gn <- graph.neighborhood(dol, order=2)
plot(gn[[2]],
     layout=layout)
dev.off()
# plot 4
...
plot(dol, 
     layout=layout,
     vertex.size = map(degree(dol),c(1,20)),
     vertex.color= map(degree(dol),c(1,20)))
dev.off()
...
# plot 7
sg1 <- cluster_spinglass(dol, spins = 3, gamma=1.0) # spin is the number of cummunity
jpeg(filename="dolphins_commu3.jpg", width=800, height=800,units='px')
plot(dol,
     layout=layout,
     vertex.size=5,
     vertex.color=rainbow(10,.8,.8,alpha=0.8)[sg1$membership])
dev.off()

# plot 8
sg1 <- cluster_spinglass(dol, spins=3, gamma=1.0)
jpeg(filename='dolphins_subcommu.jpg', width=800, height=800, units='px')
subg <- induced.subgraph(dol, which(membership(sg1)==1))
